...
class britishcorner(scrapy.Spider):
    name = 'britishcorner'
    custom_settings = {'CONCURRENT_REQUESTS': 30,
                       'FEED_FORMAT': 'csv',
                       'FEED_URI': datetime.now().strftime('%Y_%m_%d__%H_%M') + 'britishcorner.csv',
                       'RETRY_TIMES': 5,
    ...
    def __init__(self, cat=None,id =None, *args, **kwargs):
        super(britishcorner, self).__init__(*args, **kwargs)
        self.cat = cat
        if not id:
            self.id = '788'
        else:
    ...
    def parse_details(self, response):
        ...
        item = {
            ...
            'Image Path' : 'Britishcorner/'+image_link.split('/')[-1].split('.')[0]+'.'+image_link.split('.')[-1].split('?')[0],
