...
class gsheet_morrisons(scrapy.Spider):
    name = 'gsheet_morrisons'
    custom_settings = {'CONCURRENT_REQUESTS': 30,
                       'FEED_FORMAT': 'csv',
                       'FEED_URI': datetime.now().strftime('%Y_%m_%d__%H_%M') + 'gsheet_morrisons.csv',
                       'RETRY_TIMES': 5,
        ...
        for item in items:
            ...
            final_item = {
                ...
                # 'Image Path': 'images/' + image_url.split('/')[-1].split('?')[0],
                 'Image Path': 'images/' + sku + '.' + image_url.split('.')[-1].split('?')[0],
                'Category': category,
